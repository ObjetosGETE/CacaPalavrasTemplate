...
namespace PluginMaster
{
    public class PlayModeSave : EditorWindow
    {
        ...
        private static bool ValidateApplyMenu(MenuCommand command) => !Application.isPlaying && _saveData.ContainsKey(GetKey(command.context));
        ...
        private static Dictionary<(int objId, int compId), (SerializedObject serializedObj, SaveCommand saveCmd)> _saveData 
            = new Dictionary<(int objId, int compId), (SerializedObject serializedObj, SaveCommand saveCmd)>();

        private static (int objId, int compId) GetKey(Object comp) => ((comp as Component).gameObject.GetInstanceID(), (comp as Component).GetInstanceID());
        ...
        private static void Add(Component component, SaveCommand cmd)
        {
            ...
            var key = (objId, compId);
            var data = new SerializedObject(component);
            if (_saveData.ContainsKey(key)) _saveData[key] = (data, cmd);
            else _saveData.Add(key, (data, cmd));
        ...
        {
            var ids = _saveData.Keys.ToArray();
        ...
        private static void Apply((int objId, int compId) key)
        {
            var obj = EditorUtility.InstanceIDToObject(key.objId) as GameObject;
            if (obj == null) return;
            var data = _saveData[key].serializedObj;
        ...
        private static class ApplicationEventHandler
        {
            ...
            {
                var keys = _saveData.Keys.Where(k => k.objId == instanceID).ToArray();
